...
## 2026-09-01 (2) — รายงานเงินเดือนที่พิมพ์: 0 เป็น "–" + ลงแถบสีช่องที่มีตัวเลข

**เดิม:** ทุกช่องขึ้น `0.00` เต็มไปหมด ทำให้หาช่องที่มีตัวเลขจริงยาก (งวด ส.ค. มีช่องศูนย์ 188 ช่อง)
**ใหม่** (`src/lib/print.js` → `printPayrollRegister`)
- ช่องที่เป็น 0 → `–` สีจาง (`#d6d3d1`) ไม่มีแถบสี
- ช่องที่มีตัวเลข → แถบสีอ่อนแยกตามความหมาย
  · รายรับ (ฐาน/คอมฯ/ค่าวันหยุด/โบนัส) เขียวอ่อน `#ecfdf5`
  · รวมรับ เขียวเข้มขึ้น `#d1fae5`
  · รายการหัก (ปกส./ขาด-สาย/ค่าหอ/เบิก/หักอื่น/ผ่านบริษัท) แดงอ่อน `#fef2f2`
  · สุทธิ `#a7f3d0` ตัวหนา
- **แถวยอดรวมไม่ลงแถบสี** (มีพื้นหลังของตัวเองอยู่แล้ว) แต่ใช้ `–` แทน 0 เหมือนกัน
  เพื่อให้แยกออกว่าแถวไหนคือข้อมูล แถวไหนคือสรุป

สีพิมพ์ติดจริงเพราะ `openPrintHtml` ตั้ง `print-color-adjust:exact` ไว้อยู่แล้ว

**วิธี preview ที่ใช้ (เร็วกว่ารอ deploy):** เปิดแอปบน production → stub `window.open` ดัก HTML
ที่แอปสร้าง → `document.write` ทับแท็บเดิม → ใช้ JS ไล่แก้สไตล์ตามดีไซน์ใหม่ → screenshot
ได้เห็นรายงานจริงพร้อมข้อมูลจริงก่อนส่ง patch
...
**ปัญหาลูกโซ่:** พอคอลัมน์รายการกว้างขึ้น ตารางกว้างเกินกรอบ 38px → ต้องเลื่อนแนวนอน
→ คอลัมน์ "ชื่อ" ที่ `sticky left-0` ไปบังคอลัมน์ "ฐาน" ทำให้ `22,000.00` อ่านเหลือ `00.00`
ซึ่ง**อันตรายกว่าโดนตัดท้าย** เพราะดูเหมือนตัวเลขจริง
→ แก้เป็น 2 บรรทัด (ตัวเลขบน / ชื่อล่างแบบตัดคำ) คอลัมน์เหลือ ~183px ตารางพอดีจอ ไม่ต้องเลื่อน
แถวที่มีรายการสูงขึ้นจาก 52 → 66px แลกกับการอ่านได้ครบ

**เวลาจะแก้ layout ตาราง ให้ preview บนหน้าจริงก่อน** — inject โครงใหม่เข้า DOM ด้วย JS
แล้ววัด `table.width` เทียบ `wrapper.clientWidth` เร็วกว่ารอ deploy แล้วค่อยรู้ว่าพัง
